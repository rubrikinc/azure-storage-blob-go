...
func (s *ContainerURLSuite) TestCreateDelete(c *chk.C) {
	...
	containers, err := sa.ListContainersSegment(context.Background(), azblob.Marker{}, azblob.ListContainersSegmentOptions{Prefix: containerName})
	c.Assert(err, chk.IsNil)
	c.Assert(containers.ContainerItems, chk.HasLen, 1)
	...
	containers, err = sa.ListContainersSegment(context.Background(), azblob.Marker{}, azblob.ListContainersSegmentOptions{Prefix: containerName})
	c.Assert(err, chk.IsNil)
	c.Assert(containers.ContainerItems, chk.HasLen, 0)
}
...
func (s *ContainerURLSuite) TestListBlobs(c *chk.C) {
	...
	c.Assert(blobs.Segment.BlobItems, chk.HasLen, 0)
	...
	c.Assert(blobs.Segment.BlobItems[0].Name, chk.NotNil)
	c.Assert(blobs.Segment.BlobItems[0].Snapshot == "", chk.Equals, true)
	c.Assert(blobs.Segment.BlobItems[0].Metadata, chk.HasLen, 0)
	c.Assert(blobs.Segment.BlobItems[0].Properties, chk.NotNil)
	c.Assert(blobs.Segment.BlobItems[0].Properties.LastModified, chk.NotNil)
	c.Assert(blobs.Segment.BlobItems[0].Properties.Etag, chk.NotNil)
	c.Assert(blobs.Segment.BlobItems[0].Properties.ContentLength, chk.NotNil)
	c.Assert(blobs.Segment.BlobItems[0].Properties.ContentType, chk.NotNil)
	c.Assert(blobs.Segment.BlobItems[0].Properties.ContentEncoding, chk.NotNil)
	c.Assert(blobs.Segment.BlobItems[0].Properties.ContentLanguage, chk.NotNil)
	c.Assert(blobs.Segment.BlobItems[0].Properties.ContentMD5, chk.NotNil)
	c.Assert(blobs.Segment.BlobItems[0].Properties.ContentDisposition, chk.NotNil)
	c.Assert(blobs.Segment.BlobItems[0].Properties.CacheControl, chk.NotNil)
	c.Assert(blobs.Segment.BlobItems[0].Properties.BlobSequenceNumber, chk.IsNil)
	c.Assert(blobs.Segment.BlobItems[0].Properties.BlobType, chk.NotNil)
	c.Assert(blobs.Segment.BlobItems[0].Properties.LeaseStatus, chk.Equals, azblob.LeaseStatusUnlocked)
	c.Assert(blobs.Segment.BlobItems[0].Properties.LeaseState, chk.Equals, azblob.LeaseStateAvailable)
	c.Assert(string(blobs.Segment.BlobItems[0].Properties.LeaseDuration), chk.Equals, "")
	c.Assert(blobs.Segment.BlobItems[0].Properties.CopyID, chk.IsNil)
	c.Assert(string(blobs.Segment.BlobItems[0].Properties.CopyStatus), chk.Equals, "")
	c.Assert(blobs.Segment.BlobItems[0].Properties.CopySource, chk.IsNil)
	c.Assert(blobs.Segment.BlobItems[0].Properties.CopyProgress, chk.IsNil)
	c.Assert(blobs.Segment.BlobItems[0].Properties.CopyCompletionTime, chk.IsNil)
	c.Assert(blobs.Segment.BlobItems[0].Properties.CopyStatusDescription, chk.IsNil)
	c.Assert(blobs.Segment.BlobItems[0].Properties.ServerEncrypted, chk.NotNil)
	c.Assert(blobs.Segment.BlobItems[0].Properties.IncrementalCopy, chk.IsNil)
}
...
func (s *ContainerURLSuite) TestListBlobsPaged(c *chk.C) {
	...
	for i := 0; i < iterations; i++ {
		...
		c.Assert(err, chk.IsNil)
		c.Assert(resp.Segment.BlobItems, chk.HasLen, maxResultsPerPage)
...
func (s *ContainerURLSuite) TestListBlobsWithPrefix(c *chk.C) {
	...
	c.Assert(err, chk.IsNil)
	c.Assert(blobs.Segment.BlobPrefixes, chk.HasLen, 3)
	c.Assert(blobs.Segment.BlobItems, chk.HasLen, 0)
}
